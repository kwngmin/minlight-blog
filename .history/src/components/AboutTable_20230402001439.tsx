...
export default function AboutTable({ tableData, subject }: Props) {
  return (
    <article className="my-12">
      <h2 className="text-center lg:text-left text-3xl font-semibold text-slate-600 border-b py-4">
        {subject}
      </h2>
      <ul>
        {tableData.map(({ organization, role, period }) => (
          <li
            key={organization}
            className="flex flex-col justify-center lg:justify-start lg:flex-row items-center h-28 lg:h-16 border-b gap-0 lg:gap-4 px-8"
          >
