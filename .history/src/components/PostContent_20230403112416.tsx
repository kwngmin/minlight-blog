...
export default function PostContent({ post }: { post: PostData }) {
  ...
  return (
    <section className="border py-4 px-6 sm:p-8 md:px-16 md:pb-12 lg:px-32 lg:pb-16 lg:pt-8">
      ...
      <div className="flex items-center pt-4 gap-1 text-black/40 text-sm">
        <div className="rounded-full overflow-hidden w-6 bg-zinc-400 mr-2">
          <Image src={avatar} alt="avatar" width={120} height={120} />
        </div>
        <span>Kwngmin</span>·<time>{getLocaleDateString(date)}</time>
        <time>({format(date.toString(), "ko")})</time>
      </div>
